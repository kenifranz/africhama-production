...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve') {
        ...
        if ($payment_user) {
            sendNotification($conn, $payment_user['user_id'], "Your payment has been approved.", "payment_approved", "/dashboard.php");
        }
    }

    $_SESSION['success_message'] = "Payment " . ($action === 'approve' ? 'approved' : 'rejected') . " successfully.";
    header("Location: " . $base_path . "/approve_payments.php");
    exit();
}
...
<?php
include $root_path . '/includes/admin_footer.php';
